h2. Action View Önbakış

Bu klavuzda şunları öğreneceksiniz:

* Action View nedir ve Rails ile birlikte nasıl kullanılır
* Action View Rails dışında nasıl kullanılır
* Partial'lar, layout'lar ve kalıplar en iyi nasıl kullanılır
* Action View ile hangi Helper metodlar verilir ve kendi metodunuzu nasıl yaparsınız
* Yerel görselleri nasıl kullanırsınız

endprologue.

h3. Action View Nedir?

Action View ve Action Controller action Pack'in iki ana parçasıdır. Rails içinde web çağrıları Action Pack tarafından değerlendirilir ve iki kısıma ayrılır. Lojiği yapan kontrolör parçası ve bir kalıbı yayınlamanızı sağlayan görsel parçası. Tipik olarak, Action Controller veritabanı ile haberleşme ve gerekli CRUD işlemlerini gerçekleştirmekle ilgilenecektir. Action View ise cevabı derlemekten sorumludur.

Action View kalıpları HTML içine özel bir tag ile yerleştirilen Ruby kodlarıyla oluşturulur. Bu kalıpların bir sürü kodla sayfayı karıştırmasını önlemek için bir kısım yardımcı sınıf ile formlar, tarihler ve stringler düzenlenir. Ayrıca uygulama ihtiyaç duydukça yeni yardımcı komutlar eklenebilir.

Note: Action View'in bazı özellikleri Active Record'a bağlıdır, ama bu Action View'in Active Record'a bağlı olduğu anlamına gelmez. Action View bağımsız bir pakettir ve her zeminde kullanılabilir. 

h3. Action View'in Rails ile Kullanımı

TODO...

h3. Action View'in Rails Dışında Kullanımı

Action View , Action Record ile gayet güzel çalışır fakat ayrıca diğer Ruby araçlarıyla da düzgün çalışır. Bunu Action View özelliklerini kullanan küçük bir  "Rack":http://rack.rubyforge.org/ uygulaması üreterek deneyebiliriz. Bir Rack uygulması içinden Action View yardımcılarını kullanmak isterseniz çok kullanışlı olacaktır.

İlkönce Action Pack ve Rack gemlerin yüklü olduğundan emin olalım:
...
Şimdi Active Support tarafından sağlanan +titleize+ metodunu kullanan basit bir "Hello World" uygulaması yapalım.
...
Şimdi bunları uygulamayı çalıştırıp tarayıcıda +http://localhost:4567/+ adresine giderek herşeyi görebiliriz.
...
Dikkat ettiyseniz 'hello world' +titleize+ yardımcı metoduyla 'Hello World' haline geldi.

Action View aynı yöntemle "Sinatra":http://www.sinatrarb.com/ ile de kullanılabilir.

Action Pack ve Sinatra gemlerin kurulu olduğundan emin olarak başlayalım:
...
Şimdi de aynı "Hello World" uygulamasını Sinatra ile yapalım.
...
Sonra uygulamayı çalıştıralım:

<shell>
ruby hello_world.rb
</shell>

Uygulama çalışınca Sinatra ve Action View'i +http://localhost:4567/+ adresini ziyaret ederek çalışırken görebilirsiniz.

TODO needs a screenshot? I have one - not sure where to put it.

h3. Kalıplar, Partial'lar and Yerleşimler
...
h3. Partial Yerleşimleri

Partiallar kendi yerleşimlerine sahip olabilir. Bu yerleşimler ilgili eylem için tanımlanandan tamamen farklı olabilir, fakat benzer teknikle çalışırlar.

Diyelim bir sayfada gösterim amaçlı bir +div+ içersinde bir gönderiyi göstereceğiz. Önce yeni bir +gönderi+ oluşturalım:

<ruby>
Post.create(:body => 'Partial Layouts are cool!')
</ruby>

+show+ kalıbı içinde +post+ partial'ini göstereceğiz, ve içine konduğu bölümün yerleşimi de +box+ yerleşimi olacak:
...
+box+ yerleşimi post partial'ini bir +div+ içinde saracaktır:
...
+post+ partial'i gönderinin +body+ sini +div_for+ yardımcısını kullanarak +id+ si post ile aynı olan bir +div+ içinde sarmalayacak:
...
Not olarak partial yerleşim +render+ çağrısıyla gönderilen yerel +post+ değişkenine erişebilir. Bununla beraber uygulama geneli olan yerleşimlere benzemediği yer isminin alt çizgi ile başlamasıdır.

Ayrıca partial yerleşim içinde +yield+ yerine bir kod bloğu kullanarak da görsel oluşturabilirrsiniz. Örneğin +post+ partial görseliniz yoksa bunun yerine şöyle yapabiliriz:
...
Eğer az önceki +box+ partial yerleşimi kullanırsak bu kod az evvelki ile aynı çıktıyı verecktir.

h3. View Yönlendirmeleri

TODO...

h3. Action View tarafından sağlanan tüm yardımcılara bir önbakış

Aşağıdakiler Action View tarafından sağlanan yardımcı metodların bir kısa anlatımı. Tüm yardımcıları ayrıntısıyla inceleyen API dökümanını takip etmenizi öneririz. Burası sadece başlangıç için güzel bir nokta olabilir.

h4. ActiveRecordHelper

Active Record Helper formları veritabanlarının kayıtlarına ait oluşum değişkenlerine direk bağlayabilecek kolaylıklar sağlar. Ayrıca "Rails Form helpers guide":form_helpers.html a bakmak isteyebilirsiniz.

h5. error_message_on

İlgili nesnenin belirtilen metodunda bir hata mesajı varsa string içinde geri döner.
...
Bir DIV içinde belirtilen obje için tüm hata mesajlarını geri döner.
...
Active Record nesnesinin tüm değerlerini içeren bir form geri döner. Örneğin +String+ olarak +title+ ve +Text+ olarak +body+ değerleri olan bir +@post@ düşünelim. +form+ kullanılınca bu değerleri girdi olarak alıp yeni bir gönderi oluşturacak form geri döner.
...
Tipik olarak +form+ yerine +form_for+ kullanılır. Çünkü modelin özelliklerinin hepsini otomatikman içermesi istenmeyebilir.

h5. input

Metodun döndüğü nesne tipine uygun bir input tag'i geri döner.

Örneğin, Eğer +@post+ +String+ tipinde tanımlanmış +title+ değerine sahipse ve içinde "Hello World" varsa:
...
Bu modül yardımıyla görseller ile resimler, JavaScript dosyaları, CSS dosyaları ve RSS bağlantıları yapan HTML üretilir.

Default olarak Rails bağlantıları public klasörü altındaki dosyalaradır, ama isterseniz +config/environments/production.rb+ içindeki +ActionController::Base.asset_host+ değerini değiştirebilirsiniz, mesela dosyalarınız için yeriniz +assets.example.com+ olsun:
...
Bir yada daha fazla JavaScript dosyasını bir değişken yardımıyla javascript_include_tag metoduna göndermek için kullanılır. Genellikle plugin gibi birşeyleri yüklemek amacıyla kullanılabilir.
...
RSS veya ATOM okuyucu programların otomatik algılayabileceği bir link tag üretir.
...
+public/images+ içindeki bir resim için path geri döner. document root'a göre tüm path geri döner. Dahili olarak +image_tag+ tarafından resmin yolunu belirlemek için kullanılır.
...
Bir image tag için HTML kod döner, kod resmin tüm yolunu içerebilir yada direk isimse +public/images+ klasöründedir.
...
Verilen her kaynak ismine karşı bir html script tag üretir. +public/javascripts+ klasöründen bir dosya ismini verebileceğiniz gibi tüm yolunu birlikte verebilirsiniz. Dosya isminde +.js+ eklentisini yazmaya gerek yok. 
...
Prototype ve Scriptaculous JavaScript kütüphanelerini uygulamanızda kullanmak için kaynak olarak +:defaults+ girin. Eğer  +:defaults+ kullanıyorsanız ve +public/javascripts+ klasöründe bir +application.js+ dosyanız varsa o da kullanılacaktır.

<ruby>
javascript_include_tag :defaults
</ruby>

Bundan başka +public/javascripts+ klasöründeki tüm JavaScript dosyaları +:all+ parametresi ile yükleyebilirsiniz.

<ruby>
javascript_include_tag :all
</ruby>

Ayrıca gzip ile sıkıştırılabilen ve indirmek için daha az HTML bağlantısı kullanan şekilde birçok JavaScript dosyasını birlikte kullanabilirsiniz.  +ActionController::Base.perform_caching+ değişkeni true yaparak bu sağlanır. (bu Rails production için default değerdir ama development için değil).
